...
export async function GET(request: NextRequest) {
  ...
  const { data, error } = await supabase
    ...
    .select('cedula, hora_ingreso, salida_efectiva, horas_extra_manual, configurado_por_nombre, configurado_en')
    .eq('fecha', fecha)
...
export async function POST(request: NextRequest) {
  ...
  const { cedula, fecha, hora_ingreso, salida_efectiva, horas_extra_manual, configurado_por_cedula, configurado_por_nombre } = body
