...
export async function GET(request: NextRequest) {
  ...
  const registros: RegistroHE[] = (asistencias ?? []).map(a => {
    ...
    if (a.hora_salida && turno) {
      ...
      if (turno === 'T1') {
        // Margen 15:00-15:50 (900-950). Dentro → 15:30; fuera (> 15:50) → real
        salidaEfectiva = (outMins >= 900 && outMins <= 950) ? '15:30' : a.hora_salida
      } else if (turno === 'T2') {
        // Margen 22:15-22:50 (1335-1370). Dentro → 22:30; fuera → real
    ...
    if (salidaEfectiva && salidaNormMins > 0) {
      ...
      // Recargo nocturno: desde las 19:00 hasta la salida efectiva
      // Condición >= 22:30 para incluir el caso normalizado (dentro del margen)
      if (efMins >= 22*60+30) {
